...
export default function SmallToolBar({
  ...
  return (
    <div
      ref={containerRef}
      className={`absolute  text-white rounded-2xl bg-[#1f1f2b]  shadow-xl left-25 bottom-17 flex z-20 ${tools.includes(displayTool ?? "") ? "p-2" : "hidden"}  gap-3`}
    >
      <div
        className={`
  ...
  `}
      ...
      <div className="relative w-8 h-8" onClick={() => toggle("settings")}>
        <Settings2 />
        <div
          className={`absolute text-white md:left-[150%] w-[170px] bottom-12 flex flex-col justify-center rounded-2xl bg-[#1f1f2b] shadow-xl ${panel === "settings" ? "p-4" : "hidden"} z-0`}
          onClick={(e) => e.stopPropagation()}
        >
          {panel === "settings" && (
            <>
